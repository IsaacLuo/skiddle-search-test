...
export function* fecthSearchResult(action: IAction) {
  try {
    const result = yield call(
      axios.get,
      `https://www.skiddle.com/api/v1/events/search/?api_key=${API_KEY}&keyword=${action.data}`
    );
    yield put({ type: "SET_SEARCH_RESULT", data: result.data });
  } catch (error) {
    notification.error({
      message: "Network Problem",
      description: "unable to connect the server, please try again later",
    });
  }
}
...
export function* fetchEventDetail(action: IAction) {
  try {
    const result = yield call(
      axios.get,
      `https://www.skiddle.com/api/v1/events/${action.data}/?api_key=${API_KEY}`
    );
...
export function* fetchArtistDetail(action: IAction) {
  try {
    const result = yield call(
      axios.get,
      `https://www.skiddle.com/api/v1/artist/${action.data}?api_key=${API_KEY}`
    );
